...
import { onValue, update, set, off, ref } from "firebase/database";
import { onAuthStateChanged, signInWithCredential, signInWithPopup, signOut, sendPasswordResetEmail, signInWithEmailAndPassword, signInWithCustomToken, createUserWithEmailAndPassword } from "firebase/auth";
...
export const checkUserExists = async (data) => {
  try {
    ...
    console.log("🔍 checkUserExists(): Tentando", { url, data });
    ...
    const json = await response.json();
    console.log("✅ checkUserExists(): Resposta", json);
    return json;
  } catch (error) {
    console.log("⚠️ checkUserExists(): Erro (retornando padrão)", error);
    // Em desenvolvimento, permite qualquer email/mobile
    return { exists: false };
  }
};
...
export const mainSignUp = async (regData) => {
  try {
    ...
    console.log("🔍 mainSignUp(): Iniciando cadastro", { email: regData.email, usertype: regData.usertype });
    ...
    console.log("✅ mainSignUp(): Usuário criado no Firebase Auth", { uid });
    ...
    await set(userRef, {
      ...
      usertype: regData.usertype || "customer",
      createdAt: new Date().getTime(),
      approved: 0,
      carType: regData.carType || null,
      referralId: regData.referralId || null
    });
    
    console.log("✅ mainSignUp(): Dados salvos no Realtime Database");
    
    return { uid: uid, success: true };
  } catch (error) {
    console.log("❌ mainSignUp(): Erro", error.message);
