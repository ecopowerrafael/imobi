...
function App() {
  ...
  useJsApiLoader({
    ...
    // version removido - Google Maps usa a versão mais recente automaticamente
    libraries
  })

  // 🔧 Inicializa o banco de dados na primeira carga
  useEffect(() => {
    console.log("🚀 App iniciado. Verificando banco de dados...");
    initializeDatabase();
    
    // Expõe funções de debug no console global
    window.__debugTaxiApp = {
      clearDatabase: async () => {
      ...
      reinitializeDatabase: async () => {
        console.log("🔄 Reinicializando banco...");
        await clearDatabase();
    ...
    console.log("💡 Use window.__debugTaxiApp.clearDatabase() ou window.__debugTaxiApp.reinitializeDatabase() no console");
  }, []);
